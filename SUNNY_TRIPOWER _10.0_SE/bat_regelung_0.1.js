...
const pvforecastDP = '0_userdata.0.strom.pvforecast.';
...
const communicationRegisters = {
    fedInSpntCom: 'modbus.0.holdingRegisters.3.40151_Kommunikation', // Wirk- und Blindleistungsregelung über Kommunikation (802 active, 803 inactive)
    ...
    wMaxDsch: 'modbus.0.holdingRegisters.3.40191_max_Entladeleistung_BatWR', // Max Entladeleistung BatWR
};

const inputRegisters = {
    batSoC: 'modbus.0.inputRegisters.3.30845_Batterie_Prozent', // Batterie-SoC (Selbsterklärend)
...
const    _tibber = true;                //wird _tibber benutzt (dyn. Strompreis) 
...
const    _lossfactor = 0.75;            //System gesamtverlust in % (Lade+Entlade Effizienz), nur für Awattar Preisberechnung
...
const    _stop_discharge = (_start_charge * _loadfact);
    ...
    createUserStates(tibberDP1, false, [tibberDP2 + 'extra.tibber_laden_erlauben', { 'name': 'mit tibber laden erlauben', 'type':'boolean', 'read': true, 'write': true, 'role': 'state', 'def':true }], function () {        
        setState(tibberDP + 'extra.tibber_laden_erlauben', _gridcharge, true);
    }); 
    ...
    createUserStates(tibberDP1, false, [tibberDP2 + 'extra.PV_Prognose_kurz', { 'name': 'PV_Prognose', 'type':'number', 'read': true, 'write': false, 'role': 'state', 'unit': 'kWh', 'def':0 }], function () {        
...
function processing() {
    ...
    if (_debug) { 
        console.log('Ladeleistung Batterie ' + _batteryPower + ' W'); 
        console.log('Einspeiseleistung ' + cur_power_out + ' W'); 
    }
    ...
    if (_tibber) {
        price0 = getState(tibberDP + '0.price').val;
    };  
    ...
    if (_debug) { 
        console.log('Lademenge bis voll ' + ChaEnrg_full + ' Wh') ;
        console.log('Lademenge ' + ChaEnrg + ' Wh') ;
        console.log('Restladezeit ' + ChaTm.toFixed(2) + ' h') ;
    }

// ------------------------------------------------------------------------------------------   Start der _tibber Sektion
  if (_tibber){
    let poi = [];
    for (let t = 0; t < 12 ; t++) {
    ...
    if (price0) {
        //defaults
        let dt = new Date();
  //      let nowhr = dt.getHours() + ':' + dt.getMinutes();
  //      let timeup = getDateObject(new Date().getTime()-1800000).getHours() + ':' + getDateObject(new Date().getTime()-1800000).getMinutes();
        ...
        if (pvwh > (_baseLoad*hrstorun/2) && !_snowmode) {
            ...
            if (_debug) {
                console.log('Nachtfenster:' + sundownhr + '-' + sunup + ' (' + hrstorun.toFixed(2) + ' h)');
            }
        ...
        if (_debug) { 
            console.log('Erwarte ca ' + (pvwh/1000).toFixed(1) + ' kWh von PV');
        }
        ...
        if (batlefthrs < hrstorun && _gridcharge) {
            let pricelimit = 0;
            let m = 0;
            ...
            if (batlefthrs*2 <= lefthrs) {
                ...
                for (let d = 0; d < lefthrs; d++) {
                    if (poihigh[d][0] > _stop_discharge){
                        if (_debug) {
                            console.log('Entladezeiten: ' + poihigh[d][1] +'-'+ poihigh[d][2]);
                        }
        ...
        if (price0 <= _stop_discharge) {
            if ( _debug) { 
              console.log('Stoppe Entladung, Preis unter Batterieschwelle von ' + _stop_discharge.toFixed(2) + ' ct/kWh');
            }
            maxdischrg = 0;
        }
        //ladung stoppen wenn Restladezeit kleiner Billigstromzeitfenster
        if (lowprice.length > 0 && ChaTm <= lowprice.length && _gridcharge) {
            maxchrg = 0;
            tibber_active = 1;
        };

        if (price0 < _start_charge && _gridcharge) {
          maxchrg = 0;
          maxdischrg = 0;
          tibber_active = 1;
...
// ------------------------------------------------------------------------------------------   Ende der _tibber Sektion
    ...
    if (_debug && latesttime) { 
        console.log('Abschluss bis ' + latesttime);
    }
    ...
    if (ChaTm > 0 && (ChaTm * 2) <= pvfc.length && batsoc >= batminlimit) {
        ...
        if (_debug) { 
            console.log('Überschuß ' + Math.round(get_wh) + ' Wh');            
        }
        ...
        if (ChaEnrg > get_wh && ChaEnrg > 0 && ChaTm > 0) {
            ...
            if (_debug) { 
                console.log('Verschiebe Einspeiselimit auf ' + pvlimit_calc + ' W' + ' mit mindestens ' + min_pwr + ' W');                
            }
        }
    ...
    if (maxchrg != _batteryPower || maxchrg != _lastmaxchrg || maxdischrg != maxdischrg_def || maxdischrg != _lastmaxdischrg) {
        if (_debug) {
            console.log('WR max_ladeleistung : ' + maxchrg + ' , max_entladeleistung ' + maxdischrg);
        }
...
//   803: inaktiv (Ina)
    ...
    if (SpntCom != _SpntCom_def || SpntCom != _lastSpntCom) {
        if (_debug) { 
            console.log(SpntCom + ' != ' + _SpntCom_def + ' || ' + SpntCom + ' != ' + _lastSpntCom);
            console.log('Datengesendet an WR : ' + PwrAtCom + ', ' + SpntCom ); 
        }
    ...
    if (_debug) { 
        console.log('SpntCom ' + SpntCom + ' ' + _lastSpntCom);        
        console.log('----------------------------------------------------------------');    
    }
}
